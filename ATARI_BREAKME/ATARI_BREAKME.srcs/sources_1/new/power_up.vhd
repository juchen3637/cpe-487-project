...
architecture Behavioral of power_up is
    ...
    SIGNAL flip : STD_LOGIC := '0'; --_VECTOR(1 DOWNTO 0) := "00";
...
--        end if;
    END PROCESS;
    get_power_up_type: process is
    begin
        wait until rising_edge(flip);
        choice <= count;
    end process;
    ...
    begin
        ...
        if (choice=1 or choice=2) then
            --ball_y <= ball_y + 2;
        elsif (choice=0 or choice=3) then
            --void <= '1';
        end if;
        IF (((ball_x + bsize/2) >= (bat_x - (bat_w+bat_temp))) OR
           ((bat_w+bat_temp) > bat_x and (ball_x + bsize/2) >= bat_x)) AND
           (ball_x - bsize/2) <= (bat_x + (bat_w+bat_temp)) AND
